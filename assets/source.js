...
function generateUi(title, movieResults) {
...
function loadTrending() {
    ...
    fetch(`https://api.themoviedb.org/3/trending/movie/week?api_key=${apiKey}`)
        ...
        .then((data) => {
            ...
            generateUi(`Currently trending movies`, movieResults);
        });
}
...
function search() {
    ...
    if (searchQuery.length > 0) {
        ...
        .then((data) => {
            ...
            generateUi(`Results for "${searchQuery}"`, movieResults);
        });
    } else {
        loadTrending();
        alert('You attempted to search with no query.');
    }
}